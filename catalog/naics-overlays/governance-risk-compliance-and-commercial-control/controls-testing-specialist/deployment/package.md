...
- Runtime expectation: Read-only guidance package loaded by an orchestrator that owns tenancy, routing, credentials, evidence connectors, and access to tenant control repositories.
...
- Tenant isolation: No cross-tenant sharing of control matrices, walkthrough notes, samples, evidence files, exception logs, billing records, license-governance evidence, or vendor-control reliance artifacts.
- Allowed adaptation: tenant-local control inventories, evidence repositories, sample thresholds, testing calendars, system names, escalation contacts, and approved retrieval connectors.
- Prohibited adaptation: widening the public specialty boundary, replacing the public authority baseline without review, or introducing legal interpretation, audit sign-off, procurement execution, licensing optimization, FinOps execution, or remediation ownership.
...
- Log specialist dispatch decisions, test-plan outputs, walkthrough prompts, evidence-quality findings, exception and deficiency summaries, boundary rejections, and confidence downgrades.
...
- Monitor for repeated low-confidence requests, missing populations, stale evidence repositories, recurring traceability defects, repeated fallback-mode use, and drift between testing artifacts and deployment claims.

## Rollback Path

- Immediate rollback: disable the current specialist release and route controls-testing requests to orchestrator-safe fallback handling.
- Artifact rollback: restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: revert tenant-local sample thresholds, evidence connector mappings, escalation contacts, or retrieval configuration independently of the shared public baseline.
