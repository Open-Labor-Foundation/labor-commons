...
- Runtime expectation: Read-only specialist guidance package loaded by an orchestrator that controls tenant routing, customer-evidence retrieval, audit logging, and escalation to procurement, legal, software-licensing, or technical specialists.
- Required configuration:
  - release version
  - approved source freshness policy
  - tenant-specific vendor-record and evidence connectors
  - escalation targets for procurement, legal, software licensing, security review, and finance operations
  - delegated meta-agent timeout and staleness thresholds
- Required secrets: none in the package itself; any contract-system, procurement-system, GRC-tool, or vendor-portal credentials remain orchestrator-managed.
...
- Tenant boundary: no cross-tenant sharing of vendor inventories, diligence responses, contracts, issue logs, performance scorecards, audit artifacts, or evaluation traces.
- Knowledge boundary: the public authority baseline in `manifest.yaml` is shared; customer-specific vendor evidence is injected only at request time through orchestrator-controlled access.
- Allowed adaptation: tenant terminology, vendor tiers, risk thresholds, approved connectors, reviewer routing, and approved confidence thresholds.
...
- Monitoring:
  ...
  - alert on stale source review dates, repeated missing-diligence evidence patterns, overdue vendor-remediation follow-ups, and delegated meta-agent timeout or staleness events
  - monitor repeated requests for negotiation strategy or legal drafting as routing-quality signals
- Logging:
  ...
  - record whether customer-specific vendor evidence was supplied and whether the specialist returned a low-confidence escalation
- Rollback path:
...
- Runtime prompts should remind downstream systems that vendor-management recommendations are governance guidance only and do not replace legal advice, commercial approval authority, or direct sourcing execution.
