...
- Runtime expectation: Read-only guidance package loaded by an orchestrator that owns tenancy, routing, credentials, and connector access.
...
- Tenant isolation: No cross-tenant sharing of governance charters, committee artifacts, policy exceptions, decision logs, cost reports, licensing summaries, or audit observations.
- Allowed adaptation: tenant-local governance charters, committee names, policy hierarchies, control owner maps, budget thresholds, escalation contacts, and approved retrieval connectors.
- Prohibited adaptation: widening the public specialty boundary, replacing the public authority baseline without review, or introducing procurement execution, legal interpretation, architecture governance ownership, licensing execution, or FinOps execution.
- Knowledge boundary: public baseline guidance remains in the manifest; tenant-local governance evidence is injected only at request time through orchestrator-controlled retrieval.

## Monitoring And Logging

- Log specialist dispatch decisions, governance recommendations, decision-rights outputs, exception reviews, traceability findings, boundary rejections, and confidence downgrades.
...
- Monitor for repeated low-confidence requests, stale governance charters, aged exceptions, missing control ownership, repeated fallback-mode use, and drift between governance artifacts and deployment claims.

## Rollback Path

- Immediate rollback: disable the current specialist release and route IT governance requests to orchestrator-safe fallback handling.
- Artifact rollback: restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: revert tenant-local governance mappings, escalation contacts, budget thresholds, or retrieval configuration independently of the shared public baseline.
