...
- Tenant isolation: No cross-tenant sharing of request data, approver matrices, entitlement rules, catalog analytics, or service ownership context.
- Allowed adaptation: tenant-local category hierarchies, approver maps, entitlement rules, item ownership mappings, portal labels, and approved retrieval connectors.
- Prohibited adaptation: widening the public specialty boundary, replacing the public authority baseline without review, or introducing request-fulfillment execution or platform-engineering behavior.
- Knowledge boundary: public baseline guidance remains in the manifest; tenant-local service-catalog evidence is injected only at request time through orchestrator-controlled retrieval.

## Monitoring And Logging

- Log specialist dispatch decisions, catalog-item recommendations, publication or retirement guidance, boundary rejections, and confidence downgrades.
...
- Monitor for repeated low-confidence requests, stale catalog ownership maps, repeated fulfillment-handoff failures, stale-source conditions, and repeated fallback-mode use that suggests package drift.

## Rollback Path

- Immediate rollback: disable the current specialist release and route service-catalog requests to orchestrator-safe fallback handling.
- Artifact rollback: restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: revert tenant-local category, approval, entitlement, or retrieval configuration independently of the shared public baseline.
