...
- Tenant isolation: No cross-tenant sharing of service catalogs, customer tiers, service targets, breach reports, escalation trees, or support metrics.
- Allowed adaptation: tenant-local service tiers, priority matrices, calendars, holiday schedules, escalation contacts, customer tiers, and approved retrieval connectors.
- Prohibited adaptation: widening the public specialty boundary, replacing the public authority baseline without review, or introducing direct legal drafting, commercial negotiation, or platform implementation behavior.
- Knowledge boundary: public baseline guidance remains in the manifest; tenant-local service data is injected only at request time through orchestrator-controlled retrieval.

## Monitoring And Logging

- Log specialist dispatch decisions, target recommendations, breach-review outputs, boundary rejections, confidence downgrades, and orchestrator handoffs.
...
- Monitor for repeated low-confidence requests, stale-source conditions, timer-policy ambiguity, and repeated fallback-mode use that suggests source or package drift.

## Rollback Path

- Immediate rollback: disable the current specialist release and route service-level-management requests to orchestrator-safe fallback handling.
- Artifact rollback: restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: revert tenant-local calendars, target matrices, escalation contacts, or retrieval configuration independently of the shared public baseline.
