...
- Tenant isolation: No cross-tenant sharing of incident records, impact data, service ownership maps, escalation trees, or communication artifacts.
- Allowed adaptation: tenant-local priority matrices, resolver-group mappings, escalation contacts, communication templates, and approved retrieval connectors.
- Prohibited adaptation: widening the public specialty boundary, replacing the public authority baseline without review, or introducing direct technical-remediation behavior.
...
- Log specialist dispatch decisions, priority recommendations, boundary rejections, confidence downgrades, and orchestrator handoffs.
...
- Monitor for repeated low-confidence requests, duplicate routing failures, stale-source conditions, and repeated fallback-mode use that suggests source or package drift.

## Rollback Path

- Immediate rollback: disable the current specialist release and route incident-management requests to orchestrator-safe fallback handling.
- Artifact rollback: restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: revert tenant-local priority maps, escalation contacts, or retrieval configuration independently of the shared public baseline.
