...
- Tenant boundary: no cross-tenant sharing of asset inventories, CI data, service maps, ticket content, policy exceptions, or evaluation traces.
- Knowledge boundary: the public authority baseline in `manifest.yaml` is shared; tenant-local inventory and ticket context are injected only at request time through orchestrator-controlled access.
- Allowed adaptation: tenant terminology, approved connectors, risk-threshold tuning, and human-approval checkpoints.
...
- Monitoring:
  - log dispatch decisions, boundary rejections, low-confidence escalations, fallback activation, and repeated asset/CI data-quality failure modes
  - alert on stale source review dates, repeated duplicate-CI conflict patterns, and delegated meta-agent timeout or staleness events
- Logging:
