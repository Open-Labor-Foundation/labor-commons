...
- Runtime expectation: Read-only guidance package loaded by an orchestrator that owns tenancy, routing, credentials, connector access, and any live bridge or ticket integrations.
...
- Tenant isolation: No cross-tenant sharing of incident timelines, stakeholder rosters, escalation trees, service maps, bridge transcripts, or decision logs.
- Allowed adaptation: tenant-local severity matrices, stakeholder groups, escalation contacts, communications templates, service-ownership maps, and approved retrieval connectors.
- Prohibited adaptation: widening the public specialty boundary, replacing the public authority baseline without review, or introducing direct technical-remediation or cybersecurity-response behavior.
- Knowledge boundary: public baseline guidance remains in the manifest; tenant-local incident evidence is injected only at request time through orchestrator-controlled retrieval.

## Monitoring And Logging

- Log specialist dispatch decisions, declaration recommendations, command-role guidance, communications recommendations, boundary rejections, and confidence downgrades.
...
- Immediate rollback: disable the current specialist release and route major-incident requests to orchestrator-safe fallback handling.
- Artifact rollback: restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: revert tenant-local severity maps, stakeholder contacts, escalation trees, or retrieval configuration independently of the shared public baseline.
