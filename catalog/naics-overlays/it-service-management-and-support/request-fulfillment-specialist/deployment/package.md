...
- Required configuration: Confidence thresholds, request-priority matrix, fulfillment-group mappings, approval-policy references, source freshness policy, delegated meta-agent timeout thresholds, and release-version pinning.
...
- Tenant isolation: No cross-tenant sharing of request records, requester identity details, approval chains, fulfillment-task history, or catalog configuration.
- Allowed adaptation: Tenant-local request types, approval mappings, SLA targets, notification templates, portal labels, and approved retrieval connectors.
- Prohibited adaptation: Widening the public specialty boundary, replacing the public authority baseline without review, or introducing direct implementation or provisioning behavior.
- Knowledge boundary: Public baseline guidance remains in the manifest; tenant-local request evidence is injected only at request time through orchestrator-controlled retrieval.

## Monitoring And Logging

- Log specialist dispatch decisions, priority or due-date recommendations, approval assumptions, boundary rejections, confidence downgrades, and orchestrator handoffs.
...
- Monitor for repeated low-confidence requests, SLA-aging spikes, duplicate routing failures, stale-source conditions, and repeated fallback-mode use that suggests package drift.

## Rollback Path

- Immediate rollback: Disable the current specialist release and route request-fulfillment work to orchestrator-safe fallback handling.
- Artifact rollback: Restore the prior validated package version and re-run local repository validation before re-enabling.
- Adaptation rollback: Revert tenant-local request types, approval mappings, notification templates, or retrieval configuration independently of the shared public baseline.
