...
- Package type: validated spec pack under `catalog/cloud-platform-and-infrastructure/aws-architecture-specialist/`.
...
- Required secrets: none for base spec-pack use. Tenant-specific AWS read access is optional and must be supplied through approved retrieval integrations with tenant isolation and audit logging.

## Materialization Requirements

A runtime materializer must supply or retrieve:

- AWS organization, account, OU, region, Availability Zone, shared-services, landing-zone, and guardrail records.
- VPC, Transit Gateway, endpoint, DNS, hybrid connectivity, routing, firewall, security group, and network inventory when topology guidance is requested.
...
- CloudWatch, CloudTrail, AWS Config, AWS Backup, Cost Explorer, Well-Architected Tool, incident, runbook, ADR, diagram, CMDB, IaC repository, CloudFormation, CDK, Terraform state, and change-record evidence.
- Source precedence between AWS public guidance, tenant standards, customer contracts, regulatory obligations, and human owner decisions.
...
The materialized specialist may recommend AWS architecture options, risks,
tradeoffs, implementation sequencing, validation checkpoints, and owner
...
- enterprise identity policy ownership without AWS architecture coupling
- network operations execution, carrier troubleshooting, or packet-level implementation
- Kubernetes or container platform operations after AWS account and network architecture are no longer the decision surface
...
- Do not reuse tenant diagrams, account inventories, Terraform state, CloudFormation events, incident records, cost evidence, or policies across tenants.
...
Invocation logs should include task class, scope verdict, source IDs, tenant evidence references, confidence state, refusal or handoff reason, and output shape. Logs must not store secrets or cross-tenant data.

## Rollback Path

1. Disable the materialized runtime or newest spec-pack release in the dispatcher.
2. Route AWS architecture requests to the orchestrator or a previous validated package version.
...
Rollback should also remove any trusted-build claim if the package fails boundary, evidence, source freshness, or evaluation gates.

## Promotion Criteria

Keep this package as `specification-based` unless all of the following are true:

- usage volume proves the AWS architecture specialist is called frequently enough that retaining a generated implementation materially improves cost, latency, or reliability
...
- a human maintainer approves promotion and records the trusted-build decision separately from issue #1633

## Deployment Gaps

- Live AWS retrieval connectors are tenant-dependent and not part of this package.
...
- AWS service availability, quotas, pricing, and preview-feature behavior must be refreshed at runtime when they materially affect the recommendation.
