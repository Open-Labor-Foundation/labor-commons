...
Validate that the Education compliance specialist can execute onboarding, enablement, and enrollment-compliance workflows with strict evidence discipline, safe boundaries, and escalation when authority or policy interpretation is outside scope.
...
- Input: A training coordinator provides enrollment, program, student/faculty, consent, and study references plus SIS/LMS/docs system readiness and credential status for a program rollout.
- Expected behavior: Classify in-scope, validate prerequisites, and return an onboarding readiness disposition with evidence-backed controls and owners.
...
- Input: A request asks for compliance readiness on study-linked learners but omits FERPA release, consent records, and assessment linkage.
...
- Input: A user is missing mandatory credentialing proofs, role grants, and onboarding training artifacts for administration controls.
...
- Input: Operations requests immediate go-live while readiness evidence, rollback checks, and approval trail remain unverified.
...
- Input: A request combines initial onboarding compliance and steady-state instructional workflow support.
...
- Input: A policy-controlled training process is blocked because accreditation review evidence and control-check artifacts are absent.
...
- Input: Enrollment and study workflow prerequisites are met but departmental/grants or research-review approval for student records is still pending.
...
- Input: User asks specialist to finalize grading outcomes, issue ethics-style determination, or approve instructional interventions.
...
- Input: A study consent status or ethics review state switches from approved to pending while readiness work is in progress.
...
- Input: Public policy interpretation from FERPA implies one readiness outcome while tenant operational rules require stricter approval and documentation.
...
- Input: The request contains limited context for approval matrix, exception ownership, and record-system source precedence.
...
- Input: A campus uses facility-specific terms for cohorts, instruction cohorts, and compliance reviews that alter standard controls.
